...
function FeedbackTab() {
  ...
  async function pickWinners() {
    ...
    const r = await api("/api/feedback/pick-winners", {
      method: "POST",
      body: JSON.stringify({ count, ids }),
    });
    ...
    <div className="tab-content">
      ...
      <div className="card winner-controls">
        <div className="winner-ctrl-row">
          <div>
            <div className="ctrl-label">Aantal winnaars kiezen</div>
            <div className="ctrl-desc">
              {selected.size > 0 ? `${selected.size} geselecteerde items` : "Willekeurig uit alle feedback"}
            </div>
          </div>
          <div className="number-input">
            {[1,2,3,5].map(n => (
          ...
          <button className="btn-primary" onClick={pickWinners}>
            <Icons.shuffle /> Kies winnaars
          </button>
        </div>
        ...
        {winners && (
          <div className="winners-result">
            ...
            <button className="btn-sm" onClick={() => setWinners(null)}>Sluiten</button>
          </div>
        )}
      </div>
        ...
        <div className="card">
          <div className="feedback-list">
            {feedbacks.map(f => (
              <div
                key={f.id}
                className={`feedback-row ${selected.has(f.id) ? "selected" : ""}`}
            ...
            {feedbacks.length === 0 && <div className="empty">Nog geen feedback ontvangen</div>}
          </div>
        </div>
      )}
    </div>
